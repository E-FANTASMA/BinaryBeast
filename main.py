...
with col1:
    ...
    with st.form("add_investment"):
        ...
        shares = st.number_input("Number of Shares/Units", min_value=0.0, value=1.0, step=0.1)
        purchase_price = st.number_input("Purchase Price ($)", min_value=0.0, value=100.0, step=0.01)
        ...
        if st.form_submit_button("➕ Add to Portfolio"):
            ...
            st.success(f"Added {shares} shares of {asset_to_add} to portfolio!")
...
if st.session_state.portfolio:
    ...
    for asset, investments in st.session_state.portfolio.items():
        try:
            ...
            st.error(f"Error calculating portfolio for {asset}: {e}")
    ...
    if portfolio_data:
        ...
        if len(portfolio_data) > 0:
            ...
            for item in portfolio_data:
                asset_value = float(item['Current Value'].replace('$', '').replace(',', ''))
